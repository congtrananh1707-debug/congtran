...
export default function HealthChart({ memberId }: Props) {
  // Lấy toàn bộ mảng (stable reference) rồi filter bên ngoài selector
  // để tránh vô hạn re-render do filter() luôn tạo mảng mới
  const allHealth = useStore((s) => s.health)
  const addHealthRecord    = useStore((s) => s.addHealthRecord)
  const removeHealthRecord = useStore((s) => s.removeHealthRecord)

  const health = useMemo(
    () => allHealth.filter((h) => h.memberId === memberId).sort((a, b) => a.date.localeCompare(b.date)),
    [allHealth, memberId]
  )
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-bold text-gray-700 dark:text-gray-200">📊 Theo dõi chiều cao & cân nặng</h3>
        <button onClick={() => setShowForm(!showForm)}
          className="text-sm bg-violet-100 text-violet-700 px-3 py-1.5 rounded-xl font-medium hover:bg-violet-200">
          + Thêm
        </button>
      </div>
      ...
      {showForm && (
        <div className="bg-violet-50 border border-violet-200 rounded-2xl p-4 mb-4 grid grid-cols-3 gap-3">
          ...
          <div>
            ...
            <input type="number" placeholder="120" value={form.height}
              onChange={(e) => setForm({ ...form, height: e.target.value })}
              className="w-full border border-gray-200 rounded-xl p-2 text-sm" />
          </div>
          <div>
            <label className="text-xs text-gray-500 block mb-1">Cân nặng (kg)</label>
            <input type="number" placeholder="25" value={form.weight}
        ...
        <>
          ...
          <div className="mt-3 space-y-2">
            {[...health].reverse().slice(0, 5).map((h) => (
              <div key={h.id} className="flex items-center justify-between text-sm bg-gray-50 dark:bg-gray-700 rounded-xl px-3 py-2">
                <span className="text-gray-500 dark:text-gray-400">{h.date}</span>
