...
export default function SkillTree({ memberId }: Props) {
  const skills = useStore((s) => s.skills.filter((sk) => sk.memberId === memberId))
  ...
  const catSkills = skills.filter((sk) => sk.category === activeCategory)
  const totalAchieved = skills.filter((sk) => sk.achieved).length
  ...
  return (
    <div>
          ...
          return (
            <button
              ...
              className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-sm font-medium transition-all ${
                activeCategory === cat.key ? 'bg-violet-600 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
              }`}
            >
              {cat.icon} {cat.label}
              {total > 0 && <span className={`text-xs px-1.5 py-0.5 rounded-full ${activeCategory === cat.key ? 'bg-white/20' : 'bg-gray-200'}`}>{count}/{total}</span>}
            </button>
          )
        })}
      </div>
                ...
                <button
                  ...
                  className={`flex flex-col items-center gap-1 p-3 rounded-2xl w-24 transition-all hover:scale-105 border-2 ${
                    sk.achieved
                      ...
                      : 'bg-white border-gray-200 text-gray-500 opacity-70'
                  }`}
                >
      ...
      <div className="mt-3">
        {showAdd ? (
          <div className="bg-gray-50 rounded-2xl p-3 flex gap-2 items-end flex-wrap">
            <div className="flex-1 min-w-32">
              <label className="text-xs text-gray-500 block mb-1">Tên kỹ năng</label>
              <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                placeholder="VD: Biết bơi..." className="w-full border border-gray-200 rounded-xl p-2 text-sm" />
            </div>
            <div>
            ...
            <button onClick={() => setShowAdd(false)} className="text-gray-400 px-3 py-2 rounded-xl text-sm hover:bg-gray-100">Hủy</button>
          </div>
        ) : (
